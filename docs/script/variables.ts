...
const dynamicStyleMark = `tzg`;

function getStyle(globalPrefixCls: string, theme: Theme) {
    ...
    const cssList = Object.keys(variables).map(
        key => `--${globalPrefixCls}-${key}: ${variables[key]}!important;`,
    );

    return `:root {${cssList.join('')}}`.trim();
}

/**
 * 设置CSS变量
 *
 * @param {Theme} theme 样式主题，参考 antd 的动态主题配置 ConfigProvider
 * @param {string} prefix 前缀，默认：“ant”
 */
const variables = (theme: Theme, prefix: string = 'ant') => {
    const style = getStyle(prefix, theme);
